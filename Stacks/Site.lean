...
structure Pullback.{u, v} {C : Type v} [Category.{u, v} C]
          {dst : C} (f : Over dst) (g : Over dst) where
  obj : C
  fst : obj ⟶ f.left
  snd : obj ⟶ g.left
  is_pullback : IsPullback fst snd f.hom g.hom

class HasPullbacks'.{u, v} (C : Type v) [Category.{u, v} C] where
  pullback {X : C} (f : Over X) (g : Over X) : Pullback f g
...
structure Site.{u, v} (C : Type v) [Category.{u, v} C] [HasPullbacks'.{u, v} C] where
  ...
  pullback {X : C} (f : Over X) (U : Precover X) (hU : U ∈ coverings X) :
    { Over.mk (HasPullbacks'.pullback g f).snd | g ∈ U } ∈ coverings f.left
